...
if (isset($_POST["end_created_session"])) {
    ...
    redirect_with_context("create_session.php");
}
...
if (isset($_POST['create'])) {
    ...
    if (!$locationConfirmed || $venueAccuracy === false || $venueAccuracy === null) {
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            $qr_link = app_base_url() . "/mark_attendance.php?token=" . urlencode($token);
...
if (!$createdSession && !isset($_POST["create"])) {
    ...
    if ($activeSession) {
        $qr_link = app_base_url() . "/mark_attendance.php?token=" . urlencode($activeSession["session_token"]);
...
<head>
    ...
    <link rel="stylesheet" href="assets/css/style.css?v=session-persistent-1">
</head>
...
<div class="session-shell">
    ...
    <div class="session-hero">
        ...
        <a href="lecturer_dashboard.php" class="button-link secondary-action">Back to Dashboard</a>
    </div>
        ...
        <section class="live-session-card" id="liveSessionCard">
            ...
            <div class="live-qr-layout">
                <div class="live-qr-frame">
                    <img src="qr_code.php?data=<?php echo urlencode($qr_link); ?>" alt="Attendance session QR code">
                </div>
...
if (markedCount && liveSessionCard) {
    const sessionStatusUrl = <?php echo json_encode($createdSession ? with_context("session_status.php?session_id=" . (int) $createdSession["id"]) : ""); ?>;
...
if (extendSessionButton && extendMinutes) {
    extendSessionButton.addEventListener("click", async function () {
        ...
        try {
            const response = await fetch(<?php echo json_encode(with_context("session_extend.php")); ?>, {
                method: "POST",
